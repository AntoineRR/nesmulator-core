...
impl Ppu {
    ...
    pub fn clock(&mut self) {
        ...
        if self.scanline == 261 && self.cycles == 1 {
            self.registers.set_status_flag(StatusFlag::VBlank, false);
            self.registers
                .set_status_flag(StatusFlag::Sprite0Hit, false);
            self.registers
                .set_status_flag(StatusFlag::SpriteOverflow, false);
        }
        ...
        if (self.scanline < 240) && (self.cycles >= 1 && self.cycles < 257) {
            ...
            if self.registers.get_mask_flag(MaskFlag::ShowSprites) {
                ...
                for i in 0..self.current_sprite_count {
                    if self.sprite_x[i as usize] == 0 {
                        ...
                        fg_priority = (self.sprite_attributes[i as usize]
                            & (SpriteAttribute::Priority as u8))
                            == 0;
            ...
            if bg_pattern == 0 && fg_pattern == 0 {
                ...
                // Detect sprite 0 hit
                if self.current_contains_sprite_0
                    && self.is_sprite_0_rendered
                    && self.registers.get_mask_flag(MaskFlag::ShowBackground)
                    ...
                    && self.cycles != 256
                {
                    if !self
                        .registers
                        .get_mask_flag(MaskFlag::ShowLeftScreenBackground)
                        || !self
                            .registers
                            .get_mask_flag(MaskFlag::ShowLeftScreenSprites)
                    {
                        if self.cycles >= 9 {
    ...
    fn evaluate_sprites(&mut self) {
        ...
        if (self.cycles - 65) % 3 == 0 {
                ...
                && self.scanline % 261 < (self.oam.primary[sprite_index].y as u16) + sprite_size
            {
                ...
                if self.next_sprite_count >= 8 {
                    ...
                    {
                        self.registers
                            .set_status_flag(StatusFlag::SpriteOverflow, true);
                    }
                } else if self.scanline != 261 {
                    self.oam.secondary[self.next_sprite_count as usize] =
                        self.oam.primary[sprite_index];
                    if sprite_index == 0 {
        ...
        if self.cycles == 256 && self.next_sprite_count < 8 {
            self.oam
                .write_secondary(self.next_sprite_count * 4, self.oam.primary[63].y);
        }
    }
        ...
        if (sprite_index as u8) < self.next_sprite_count {
            match (self.cycles - 257) % 8 {
                ...
                0 => {
                    ...
                    let v_flip: bool = self.oam.secondary[sprite_index]
                    ...
                    if self.registers.get_control_flag(ControlFlag::SpriteSize) == 0 {
                        ...
                        if !v_flip {
                            lo_address = ((self
                                .registers
                                .get_control_flag(ControlFlag::SpritePatternTableAddress)
                                as u16)
                                << 12)
                                | ((self.oam.secondary[sprite_index].id as u16) << 4)
                                | (self.scanline as i16
                                    - (self.oam.secondary[sprite_index].y as i16))
                                    as u16;
                        }
                    ...
                    // 8x16 sprites
                        ...
                        if !v_flip {
                            ...
                            if self.scanline - (self.oam.secondary[sprite_index].y as u16) < 8 {
                                lo_address = (((self.oam.secondary[sprite_index].id & 0x01)
                                    as u16)
                                    << 12)
                            ...
                            // Second half of the sprite
                                ...
                                lo_address = (((self.oam.secondary[sprite_index].id & 0x01)
                                    as u16)
                                    << 12)
                                    | ((((self.oam.secondary[sprite_index].id & 0xFE) as u16) + 1)
                                        << 4)
                                    | ((self.scanline as i16
                                        - (self.oam.secondary[sprite_index].y as i16))
                                        & 0x07) as u16;
    ...
    fn load_next_background(&mut self) {
        ...
        if (self.next_attribute_table_byte & 0x03) == 0x00 {
            self.palette_shifters[0] &= 0xFF00;
            self.palette_shifters[1] &= 0xFF00;
    ...
    fn increment_x(&mut self) {
            ...
            || self.registers.get_mask_flag(MaskFlag::ShowBackground)
        {
            ...
            if x == 31 {
                self.ppu_bus
                    .vram_address
                    .set_address_part(VRAMAddressMask::CoarseXScroll, 0);
                let nametable_x: u16 = self
                    .ppu_bus
                    .vram_address
                    .get_address_part(VRAMAddressMask::NametableX);
                self.ppu_bus
                    .vram_address
                    .set_address_part(VRAMAddressMask::NametableX, (nametable_x == 0) as u16);
            } else {
                self.ppu_bus
                    .vram_address
                    .set_address_part(VRAMAddressMask::CoarseXScroll, x + 1);
            }
        }
    }
    ...
    fn increment_y(&mut self) {
            ...
            || self.registers.get_mask_flag(MaskFlag::ShowBackground)
        {
            ...
            if y >= 7 {
                self.ppu_bus
                    .vram_address
                    .set_address_part(VRAMAddressMask::FineY, 0);
                let c_y: u16 = self
                    .ppu_bus
                    .vram_address
                    .get_address_part(VRAMAddressMask::CoarseYScroll);
                if c_y == 29 {
                    self.ppu_bus
                        .vram_address
                        .set_address_part(VRAMAddressMask::CoarseYScroll, 0);
                    let nametable_y: u16 = self
                        .ppu_bus
                        .vram_address
                        .get_address_part(VRAMAddressMask::NametableY);
                    self.ppu_bus
                        .vram_address
                        .set_address_part(VRAMAddressMask::NametableY, (nametable_y == 0) as u16);
                } else if c_y == 31 {
                    self.ppu_bus
                        .vram_address
                        .set_address_part(VRAMAddressMask::CoarseYScroll, 0);
                    ...
                    self.ppu_bus
                        .vram_address
                        .set_address_part(VRAMAddressMask::CoarseYScroll, c_y + 1);
                }
            } else {
                self.ppu_bus
                    .vram_address
                    .set_address_part(VRAMAddressMask::FineY, y + 1);
            }
        }
    }
    ...
    fn copy_tmp_x_to_vram_address(&mut self) {
            ...
            || self.registers.get_mask_flag(MaskFlag::ShowBackground)
        {
            ...
            let tmp_c_x: u16 = self
                .ppu_bus
                .tmp_vram_address
                .get_address_part(VRAMAddressMask::CoarseXScroll);
            self.ppu_bus
                .vram_address
                .set_address_part(VRAMAddressMask::CoarseXScroll, tmp_c_x);
            // Set nametable x to tmp value
            let tmp_nt_x: u16 = self
                .ppu_bus
                .tmp_vram_address
                .get_address_part(VRAMAddressMask::NametableX);
            self.ppu_bus
                .vram_address
                .set_address_part(VRAMAddressMask::NametableX, tmp_nt_x);
        }
    }
    ...
    fn copy_tmp_y_to_vram_address(&mut self) {
            ...
            || self.registers.get_mask_flag(MaskFlag::ShowBackground)
        {
            ...
            let tmp_c_y: u16 = self
                .ppu_bus
                .tmp_vram_address
                .get_address_part(VRAMAddressMask::CoarseYScroll);
            self.ppu_bus
                .vram_address
                .set_address_part(VRAMAddressMask::CoarseYScroll, tmp_c_y);
            // Set fine y to tmp value
            let tmp_f_y: u16 = self
                .ppu_bus
                .tmp_vram_address
                .get_address_part(VRAMAddressMask::FineY);
            self.ppu_bus
                .vram_address
                .set_address_part(VRAMAddressMask::FineY, tmp_f_y);
            // set nametable y to tmp value
            let tmp_nt_y: u16 = self
                .ppu_bus
                .tmp_vram_address
                .get_address_part(VRAMAddressMask::NametableY);
            self.ppu_bus
                .vram_address
                .set_address_part(VRAMAddressMask::NametableY, tmp_nt_y);
        }
    }
    ...
    pub fn get_tile(
        ...
        if n_tile_x >= 16 || n_tile_y >= 16 {
            return Err("Tile coordinates must be in [0;15]".into());
